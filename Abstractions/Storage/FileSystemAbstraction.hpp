...
namespace {
    MemoryPool<StaticString::Container, APP_MAX_QUEUEABLE_EVENTS> ReadWritePool;
}
...
class FileSystemAbstraction {
    ...
    */
    virtual ErrorType readBlocking(FileSystemTypes::File &file, std::string &buffer) {
        Bytes read = 0;
        const ErrorType error = readBlocking(file, &buffer[0], buffer.size(), read);
    ...
    /// @copydoc ErrorType readBlocking(FileSystemTypes::File &file, std::string &buffer)
    virtual ErrorType readBlocking(FileSystemTypes::File &file, StaticString::Container &buffer) {
        Bytes read = 0;
        const ErrorType error = readBlocking(file, &buffer[0], buffer->size(), read);
    ...
    /// @copydoc ErrorType readBlocking(FileSystemTypes::File &file, std::string &buffer)
    virtual ErrorType readBlocking(FileSystemTypes::File &file, char *buffer, const size_t bufferSize, Bytes &read) = 0;
    /**
     * @brief Reads data from a file.
     ...
     * @returns ErrorType::PrerequisitesNotMet if the file has not be opened or has been closed.
    */
    virtual ErrorType readNonBlocking(FileSystemTypes::File &file, std::string &buffer, std::function<void(const ErrorType error, std::string_view buffer)> callback) {
        auto read = [&, callback, buffer = std::move(buffer)]() mutable -> ErrorType {
            ErrorType error = ErrorType::Failure;

            assert(nullptr != callback);

            error = readBlocking(file, buffer);
            callback(error, std::string_view(buffer.data(), buffer.size()));

            return error;
        };

        EventQueue::Event event = EventQueue::Event(read);
        return _storage.addEvent(event);
    }
    /// @copydoc ErrorType readNonBlocking(FileSystemTypes::File &file, std::string &buffer, std::function<void(const ErrorType error, std::string_view buffer)> callback)
    virtual ErrorType readNonBlocking(FileSystemTypes::File &file, StaticString::Container &buffer, std::function<void(const ErrorType error, std::string_view buffer)> callback) {
        ErrorType error = ErrorType::NoMemory;
        StaticString::Container* poolBuffer = nullptr;

        if (ReadWritePool.allocate(poolBuffer) == ErrorType::Success) {
            *poolBuffer = std::move(buffer);

            auto read = [&, callback, poolBuffer]() -> ErrorType {
                ErrorType error = ErrorType::Failure;

                assert(nullptr != callback);

                error = readBlocking(file, *poolBuffer);
                callback(error, std::string_view(poolBuffer->getConst()->c_str(), poolBuffer->getConst()->size()));
            ...
            if (ErrorType::Success != (error = _storage.addEvent(event))) {
                ReadWritePool.deallocate(poolBuffer);
            }
        }
    ...
    */
    virtual ErrorType writeBlocking(FileSystemTypes::File &file, std::string &data) {
        return writeBlocking(file, std::string_view(data.data(), data.size()));
    }
    /// @copydoc ErrorType writeBlocking(FileSystemTypes::File &file, std::string_view data)
    virtual ErrorType writeBlocking(FileSystemTypes::File &file, StaticString::Container &data) {
        return writeBlocking(file, std::string_view(data->data(), data->size()));
    }
    /// @copydoc ErrorType writeBlocking(FileSystemTypes::File &file, std::string_view data)
     ...
     * @returns ErrorType::Failure if the data could not be written for any other reason.
    */
    virtual ErrorType writeNonBlocking(FileSystemTypes::File &file, std::string_view data, std::function<void(const ErrorType error, const Bytes bytesWritten)> callback) {
        auto write = [&, callback, data = std::move(data)]() mutable -> ErrorType {
            ErrorType error = ErrorType::Failure;

            assert(nullptr != callback);

            error = writeBlocking(file, data);
            callback(error, data.size());

            return error;
        };

        EventQueue::Event event = EventQueue::Event(write);
        return _storage.addEvent(event);
    }
    /// @copydoc ErrorType writeNonBlocking(FileSystemTypes::File &file, std::string_view data, std::function<void(const ErrorType error, const Bytes bytesWritten)> callback)
    virtual ErrorType writeNonBlocking(FileSystemTypes::File &file, StaticString::Container &data, std::function<void(const ErrorType error, const Bytes bytesWritten)> callback) {
        ErrorType error = ErrorType::NoMemory;
        StaticString::Container* poolBuffer = nullptr;

        if (ReadWritePool.allocate(poolBuffer) == ErrorType::Success) {
            *poolBuffer = std::move(data);

            auto write = [&, callback, poolBuffer]() -> ErrorType {
                ErrorType error = ErrorType::Failure;

                assert(nullptr != callback);

                error = writeBlocking(file, *poolBuffer);
                callback(error, poolBuffer->getConst()->size());
