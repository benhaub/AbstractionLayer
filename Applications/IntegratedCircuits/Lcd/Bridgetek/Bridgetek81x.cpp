...
ErrorType Bridgetek81x::init(const SpiTypes::SpiParams params, bool externalClock, const Bridgetek81xTypes::SystemClockFrequency systemClockFrequency, const LcdTypes::ScreenParameters &screenParams) {
    ...
    if (params.driverConfig.clockFrequency <= 30E6 && params.driverConfig.clockFrequency <= static_cast<Hertz>(systemClockFrequency) / 2) {
        ...
        initParams.driverConfig.clockFrequency = std::min(Hertz(10E6), params.driverConfig.clockFrequency);
...
ErrorType Bridgetek81x::toggleBacklight(const bool on, const Percent brightness) {
    ...
    constexpr Count maxRetries = 100;
    ErrorType error;
    StaticString::Container dataBytes = StaticString::Container(std::integral_constant<size_t, sizeof(uint32_t)>());
    ...
    if (ErrorType::Success == (error = hostMemoryWrite(gpioXDirectionAddress, dataBytes, timeout, maxRetries))) {
        ...
        if (ErrorType::Success == (error = hostMemoryWrite(setBacklightAddress, dataBytes, timeout, 0))) {
...
ErrorType Bridgetek81x::calibrate(const Coordinate &instructionTextLocation) {
    ...
    if (ErrorType::Success == error) {
        ...
        if (ErrorType::Success == error) {
            ...
            if (ErrorType::Success == error) {
                constexpr std::string_view calibrationInstructionText = "Tap the dots to calibrate";
                error = drawText<calibrationInstructionText.size()>(instructionTextLocation, Bridgetek81xTypes::Font::Font11, 0x00FFFFFF, Bridgetek81xTypes::Options::Centered, calibrationInstructionText);
    ...
    if (_commandBuffer->size() + parameters.size()*4 + commandBytes.size() <= _commandBuffer->capacity()) {
        ...
        if (flush) {
            ...
            do {
                ...
                if (ErrorType::Success == error && _commandBuffer->size() < freeSpaceInCommandBuffer) {
                    error = hostMemoryWrite(address, _commandBuffer, timeout, 0);
                }
                else {
                    //TODO: We should block the calling task and rely on the interrupt signal to wake us back up again.
                    OperatingSystem::Instance().delay(Milliseconds(1));
                }

            } while (ErrorType::Success != error || freeSpaceInCommandBuffer < _commandBuffer->size());
...
uint16_t Bridgetek81x::commandBufferSpace(ErrorType &error) {
    ...
    if (freeSpaceInCommandBuffer == coprocessorFaultCode) {
        ...
        if (ErrorType::Success == error) {
            ...
            constexpr uint32_t pixelClockFrequencyAddress = static_cast<uint32_t>(Bridgetek81xTypes::MiscellaneousRegisters::PixelClockFrequency);
            StaticString::Container dataBytes = StaticString::Container(std::integral_constant<size_t, sizeof(uint32_t)>());
            ...
            if (ErrorType::Success == error) {
                ...
                dataBytes->clear();
                dataBytes->resize(dataBytes->capacity());
                ...
                if (ErrorType::Success == error) {
                    ...
                    if (ErrorType::Success == error) {
                        dataBytes->clear();
                        dataBytes->resize(dataBytes->capacity());
                        error = hostMemoryWrite(displayListFifoOffset, dataBytes, timeout, maxRetries);
                                ...
                                if (ErrorType::Success == error) {
                                    ...
                                    if (ErrorType::Success == error) {
                                        ...
                                        if (ErrorType::Success == error) {
                                            ...
                                            error = hostMemoryWrite(pixelClockFrequencyAddress, dataBytes, Milliseconds(500), maxRetries);
...
ErrorType Bridgetek81x::setScreenParameters(const LcdTypes::ScreenParameters &screenParams) {
    ...
    if (ErrorType::Success == error) {
        ...
        error = hostMemoryWrite(swizzleAddress, dataBytes, timeout, 0);
    }
    if (ErrorType::Success == error) {
