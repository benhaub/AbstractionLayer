...
namespace {
    constexpr inline uint32_t To4ByteBlock(const size_t offset, std::string_view text) {
        ...
        if (offset < text.size()) {
            const size_t available = std::min<size_t>(4, text.size() - offset);
            memcpy(&fourByteBlock, &text[offset], available);
        }
...
class Bridgetek81x {
    ...
    ErrorType drawText(const Coordinate location, const Bridgetek81xTypes::Font font, const HexCodeColour colour, const Bridgetek81xTypes::Options options, std::string_view text) {
        ...
        if (ErrorType::Success == error) {
            return [&]<size_t... IndexSequence>(std::index_sequence<IndexSequence...>) -> ErrorType {
                return writeToCommandBuffer(Bridgetek81xTypes::Commands::Text,
                    {
                        ...
                        To4ByteBlock(IndexSequence * 4, text)...
                    });
    ...
    ErrorType hostMemoryWrite(const uint32_t address, const StaticString::Container &data, const Milliseconds timeout, const Count maxRetries) {
        ...
        while (ErrorType::Success != error) {
            ...
            if (currentRetries < maxRetries && address >= registersStart && address < registersEnd) {
                ...
                uint32_t valueWritten = 0;
                memcpy(&valueWritten, &data[0], std::min(sizeof(valueWritten), data->size()));
                uint32_t valueMask = valueWritten;

                if (valueMask != 0) {
                    valueMask |= valueMask >> 1;
                    valueMask |= valueMask >> 2;
                    valueMask |= valueMask >> 4;
                    valueMask |= valueMask >> 8;
                    valueMask |= valueMask >> 16;

                    if (ErrorType::Success == error && (readBack & valueMask) != (valueWritten & valueMask)) {
