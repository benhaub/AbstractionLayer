...
ErrorType OperatingSystem::createThread(const OperatingSystemTypes::Priority priority, const std::array<char, OperatingSystemTypes::MaxThreadNameLength> &name, void * arguments, const Bytes stackSize, void *(*startFunction)(void *), Id &number) {
    ...
    InitThreadArgs *initThreadArgs = new InitThreadArgs {
        ...
        .threadId = &threads.at(toThreadIndex(nextThreadId)).tm4c123ThreadId,
    };
...
ErrorType OperatingSystem::currentThreadId(Id &thread) const {
    const TaskHandle_t currentHandle = xTaskGetCurrentTaskHandle();

    if (currentHandle == _timerServiceTaskHandle) {
        thread = _TimerServiceTaskReservedId;
        return ErrorType::Success;
    }

    auto it = std::find_if(threads.begin(), threads.end(), [currentHandle](const auto &thread) { return thread.tm4c123ThreadId == currentHandle; });
    if (threads.end() == it) {
...
ErrorType OperatingSystem::createTimer(Id &timer, const Milliseconds period, const bool autoReload, std::function<void(void)> callback) {
    ...
    if (nullptr != (timerHandle = xTimerCreate(nullptr, pdMS_TO_TICKS(period), autoReload, nullptr, TimerCallback))) {
        ...
        if (nullptr == _timerServiceTaskHandle) {
            static_assert(INCLUDE_xTimerGetTimerDaemonTaskHandle, "INCLUDE_xTimerGetTimerDaemonTaskHandle must be 1 in FreeRTOSConfig.h for timer task registration.");
            _timerServiceTaskHandle = xTimerGetTimerDaemonTaskHandle();
        }
