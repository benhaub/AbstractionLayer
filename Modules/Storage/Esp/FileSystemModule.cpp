...
ErrorType FileSystem::mount() {
    ...
    std::unique_ptr<EventAbstraction> event = std::make_unique<StorageAbstraction::Event<>>(std::bind(mountCallback));
...
ErrorType FileSystem::unmount() {
    ...
    std::unique_ptr<EventAbstraction> event = std::make_unique<StorageAbstraction::Event<>>(std::bind(unmountCallback));
    ErrorType error = _storage.addEvent(event);
    if (ErrorType::Success != error) {
        return error;
    }
...
ErrorType FileSystem::synchronize(const FileSystemTypes::File &file) {
    ...
    std::unique_ptr<EventAbstraction> event = std::make_unique<StorageAbstraction::Event<>>(std::bind(syncCallback));
    ErrorType error = _storage.addEvent(event);
    if (ErrorType::Success != error) {
        return error;
    }
