...
namespace Spiffs {
    ...
    ErrorType readBlocking(FILE *spiffsFile, FileSystemTypes::File &file, char *buffer, const size_t bufferSize, Bytes &read) {
        ErrorType error = ErrorType::Success;

        if (0 == fseek(spiffsFile, file.filePointer, SEEK_SET)) {
            read = fread(buffer, 1, bufferSize, spiffsFile);
