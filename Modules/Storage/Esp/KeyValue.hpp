...
namespace KeyValue {
    ...
    ErrorType readBlocking(std::unique_ptr<nvs::NVSHandle> &_handle, const FileSystemTypes::File &file, char *buffer, const size_t bufferSize, Bytes &read) {
        ...
        if (nullptr != _handle.get()) {
            read = std::min(Bytes(bufferSize), file.size);
            esp_err_t err = _handle->get_blob(file.path->c_str(), buffer, read);
