...
//            const i2c_port_num_t i2cPort = toEspPort(peripheral, error);
//
//            if (ErrorType::Success == error) {
//                i2c_master_bus_config_t controllerConfig = {
...
ErrorType I2c::txBlocking(const StaticString::Container &data, const Milliseconds timeout, const IcCommunicationProtocolTypes::AdditionalCommunicationParameters &params) {
    ...
    //StaticString::Container writeData = StaticString::Data<data->size() + 1>();
    //writeData->resize(0);
    //writeData->push_back(static_cast<char>(params.i2cRegisterAddress.value()));
    //writeData->append(data->c_str(), data->size());
